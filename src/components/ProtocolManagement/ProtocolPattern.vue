 ...
 * @LastEditors: WangXin
 * @LastEditTime: 2023-03-07 14:20:02
 * @Description:
 *
        ...
        <el-collapse accordion v-model="collapseName" @change="changeCollapse">
          <el-collapse-item class="unit-item" v-for="(item, index) in unitList" :key="item.unitUuid" :name="index">
            ...
            <div class="unit-item-content" :class="item.zoom ? 'full' : 'pd-20'">
              <el-button v-if="item.zoom" class="mg-y-10" style="position: fixed; top: 20px; z-index: 10" @click.stop="item.zoom = false">恢复</el-button>
              ...
              <vxe-table
                ...
                show-overflow
                :row-config="{ height: 90, useKey: true, isHover: true }"
                :scroll-y="{ enabled: true, gt: item.zoom ? 500 : 0 }"
                ref="xTable1"
                :height="item.zoom ? 1000 : 300"
              >
                <vxe-column type="seq" width="70" title="序号" fixed="left"></vxe-column>
                <vxe-column title="名称" width="200" fixed="left">
                  <template slot-scope="scope">
                    <el-input v-model="scope.row.paramName"></el-input>
                  </template>
                </vxe-column>
                <vxe-column title="参数key" width="200" fixed="left">
                  <template slot-scope="scope">
                ...
                <vxe-column title="操作" width="150">
                  <template slot-scope="scope">
                    <div class="operate-btn-wrap">
                      <i class="operate-btn el-icon-rank" title="按住拖拽"></i>
                      <i class="operate-btn el-icon-circle-plus" @click="addUnitItem(index, scope.rowIndex)"></i>
                      <i class="operate-btn el-icon-delete-solid" @click="deleteTableData(scope.rowIndex)"></i>
                    </div>
                  </template>
                </vxe-column>
                <vxe-column title="参数设置" width="200" v-if="activeUnitType == 'PARAM_SETTING'">
                  <template slot-scope="scope">
                    <el-cascader
                      clearable
                      v-model="scope.row.bind"
                      :placeholder="getParamsSetPlaceholder(scope.row)"
                      :props="cascaderProps"
                ...
                <vxe-column title="字节类型" width="200">
                  <template slot-scope="scope">
                ...
                <vxe-column title="参数类型" width="200">
                  <template slot-scope="scope">
                ...
                <vxe-column title="内容配置" width="200" align="center">
                  <template slot-scope="scope">
                    <div class="column flex-center" style="overflow: auto; height: 90px; flex-direction: column; padding: 10px 0; box-sizing: border-box">
                      ...
                      <template v-else>
                        <template v-if="scope.row.selectConfig && scope.row.selectConfig.length">
                          <div>
                            <p :class="item.success ? 'success' : ''" v-for="(item, index) in scope.row.selectConfig" :key="index">{{ item.key }}:{{ item.value }}</p>
                            <i class="el-icon-edit" @click="editConfig(scope.row, scope.rowIndex)"></i>
                          </div>
                        </template>
                        <i v-else class="el-icon-circle-plus" @click="editConfig(scope.row, scope.rowIndex, index)"></i>
                      </template>
                    </div>
                  </template>
                </vxe-column>
                <vxe-column title="循环次数" width="200" align="center">
                  <template slot-scope="scope">
                    <el-input-number class="mg-left-10" v-model="scope.row.loopCount" :min="1" :max="100" label="循环次数"></el-input-number>
                  </template>
                </vxe-column>
                <vxe-column title="循环配置" align="center" width="250">
                  <template slot-scope="scope">
                ...
                <vxe-column :title="activeUnitType == 'MONITOR' ? '是否显示' : '是否只读'" width="150" align="center">
                  <template slot-scope="scope">
                ...
                <vxe-column title="索引类型" align="center" min-width="200">
                  <template slot-scope="scope">
                ...
                <vxe-column title="步进" width="100">
                  <template slot-scope="scope"><el-input v-model="scope.row.valueStep"></el-input></template>
                </vxe-column>
                <vxe-column title="单位" width="100">
                  <template slot-scope="scope"><el-input v-model="scope.row.valueUnit"></el-input></template>
                </vxe-column>
                <vxe-column title="取值范围" width="200">
                  <template slot-scope="scope">
                    <div class="column">
                      <el-input style="width: 50%" v-model="scope.row.valueRange.min" type="number"></el-input>
                      <span>-</span>
                      <el-input style="width: 50%" v-model="scope.row.valueRange.max" type="number"></el-input>
                    </div>
                  </template>
                </vxe-column>
                <vxe-column title="预值范围" width="200">
                  <template slot-scope="scope">
                    <div class="column">
                      <el-input style="width: 50%" v-model="scope.row.earlyRange.min" type="number"></el-input>
                      <span>-</span>
                      <el-input style="width: 50%" v-model="scope.row.earlyRange.max" type="number"></el-input>
                    </div>
                  </template>
                </vxe-column>
                <vxe-column title="是否在拓扑图显示" align="center" width="150">
                  <template slot-scope="scope">
                    <el-switch v-model="scope.row.showGplot" active-color="#13ce66" inactive-color="#ff4949"></el-switch>
                  </template>
                </vxe-column>
                <vxe-column title="是否是虚拟字段" width="150" align="center">
                  <template slot-scope="scope">
                ...
                <div class="content">
                  ...
                  <el-form-item :label="name('form')" prop="protocolInstruct">
                    <el-cascader
                      ...
                      :options="getConfig(unitMap[activeDialogUnitType])"
                      :props="{ expandTrigger: 'hover', value: 'key', label: 'value', emitPath: false }"
                      @change="handleChange"
                    ></el-cascader>
                  </el-form-item>
            ...
            <div class="content">
              <div class="row" v-for="(item, index) in editConfigForm" :key="index">
                ...
                <div class="row-btn-wrap">
                  <el-checkbox v-model="item.success">正常</el-checkbox>
                  <el-checkbox v-model="item.warn">异常</el-checkbox>
                  <el-checkbox v-model="item.deviceWarn">系统故障</el-checkbox>
                  <!-- <el-radio-group v-model="item.success" v-if="unitList[collapseName].tableData[editContentIndex].paramType == 'STATUS'">
                    <el-radio :label="true">正常</el-radio>
                    <el-radio :label="'warn'">异常</el-radio>
                    <el-radio :label="'deviceWarn'">系统故障</el-radio>
                  </el-radio-group> -->
...
<script>
import { elementOfArray, deepCopy, randomWord } from '@/utils/utils';
...
export default {
  watch: {},
  components: { CommonDialog, cron, Draggable },
  props: {
    config: {
      type: Object,
      default: () => {},
  ...
  computed: {
    ...
    getParamsSetPlaceholder() {
      return (item) => {
        let str = '';
        if (item.bindUnitInfo) {
          str += item.bindUnitInfo.unitName;
        }
        if (item.bindBodyInfo) {
          str += item.bindBodyInfo.paramName;
        }
        return str;
      };
    },
  ...
  methods: {
    getConfig(item) {
      this.handleEmptyArray(this.config[item]);
    ...
    async lazyLoad(node, resolve) {
      if (node?.data) {
        const list = await this.searchProtocol(node.data.unitUuid);
        list.forEach((item) => {
          item.name = item.paramName;
          item.bind = [item.bindUnitUuid, item.bindBodyId];
          item.leaf = true;
        });
        node.data.bodyList = list;
        resolve(node);
        this.$forceUpdate();
      }
      // console.log(list);
    ...
    async getProtocolUnitMONITOR() {
      ...
      if (copyUnitList && copyUnitList.length > 0) {
        copyUnitList.forEach((item) => {
          ...
          item.bodyList = [];
          // if (item.bodyList && Array.isArray(item.bodyList)) {
    ...
    async searchProtocol(unitUuid, protocolUuid) {
      const { data } = await this.$api.searchProtocol({ unitUuid, protocolUuid });
      return data;
    },
    /**
    ...
    addUnitItem(index, $index) {
      console.log(index, this.unitList[index].tableData[$index]);
      const data = {
        id: index + 1,
        disabled: false,
        rowKey: randomWord(),
    ...
    async changeCollapse(e) {
      ...
      if (e === '') return;
      if (this.unitList[e]) {
        const list = await this.searchProtocol(this.unitList[e].unitUuid);
        if (list.length > 0) {
          list.forEach((item, index) => {
            item.bind = [item.bindUnitUuid, item.bindBodyId];
            item.ids = item.id;
            item.id = index + 1;
            item.rowKey = randomWord();
            item.disabled = true;
          ...
          this.unitList[e].tableData = list;
        } else {
          this.unitList[e].tableData = [];
          const protocol = {
            id: 1,
            rowKey: randomWord(),
            ids: '',
          ...
          this.unitList[e].tableData.push(protocol);
        }
      }
      this.rowDrop();
      this.$forceUpdate();
    ...
    rowDrop() {
      const tbody = document.querySelectorAll('.unit-list .unit-item-content .vxe-table .vxe-table--main-wrapper tbody');
      tbody.forEach((item, index) => {
        // console.log(item);
        const _this = this;
        Sortable.create(item, {
          handle: '.unit-item-content .vxe-body--row i.el-icon-rank',
          draggable: '.unit-item-content .vxe-body--row',
          ...this.dropConfig,
          // onEnd(e) {
          //   console.log(e);
          // },
